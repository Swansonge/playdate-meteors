...
function TitleScene:init()
    ...
    -- create image, sprite, animator objects for "meteor"
    self.meteorSize = 30
    ...
    self.meteorAnimator = gfx.animator.new(10000, 0.0, 1.0, pd.easingFunctions.linear)

    -- create image, sprite, animator objects for "player"
    self.playerSide = 50
    ...
    self.playerAnimator = gfx.animator.new(10000, 0.0, 1.0, pd.easingFunctions.linear)
...
function TitleScene:update()
    ...
    if self.animating then
        ...
        -- use the animator to make the meteor fade in. The alpha value provides the "fade"
        local meteorAlpha = self.meteorAnimator:currentValue()
        local fadedMeteor = self.meteorImage:fadedImage(meteorAlpha, gfx.image.kDitherTypeAtkinson)
        self.meteorSprite:setImage(fadedMeteor)

        -- use the animator to make the player fade in. The alpha value provides the "fade"
        local playerAlpha = self.meteorAnimator:currentValue()
        local fadedPlayer = self.playerImage:fadedImage(playerAlpha, gfx.image.kDitherTypeAtkinson)
        self.playerSprite:setImage(fadedPlayer)

        if self.meteorAnimator:ended() and self.playerAnimator:ended() then
